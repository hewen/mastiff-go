...
func TestAppendToCorePackage_ReadFileError(t *testing.T) {
	err := AppendToCorePackage("non_existent.go", `import "fmt"`)
	assert.Error(t, err)
}

func TestAppendToCorePackage_InvalidMarkers(t *testing.T) {
	tmp := t.TempDir()
	file := filepath.Join(tmp, "core.go")

	content := `package core`
	assert.NoError(t, os.WriteFile(file, []byte(content), 0600))

	err := AppendToCorePackage(file, `import "fmt"`)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "markers")
}

func TestAppendToCorePackage_WriteFileError(t *testing.T) {
	tmp := t.TempDir()
	file := filepath.Join(tmp, "core.go")

	content := `// MODULE_PACKAGE_START
// MODULE_PACKAGE_END`
	assert.NoError(t, os.WriteFile(file, []byte(content), 0400)) // Read-only
