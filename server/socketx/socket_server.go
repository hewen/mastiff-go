...
func (s *SocketServer) PushTo(deviceID string, data codec.Message, callback handler.AsyncCallback) error {
	return s.handler.PushTo(deviceID, data, callback)
}

// UnbindDevice unbinds a device by ID.
func (s *SocketServer) UnbindDevice(deviceID string) {
	s.handler.UnbindDevice(deviceID)
}
